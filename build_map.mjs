...
<svg viewBox="0 0 1000 500" xmlns="http://www.w3.org/2000/svg"
    ...
    <!-- Detailed 110m World Atlas Path -->
    <path class="continent" d="${pathString}" />

    <!-- ── LOCATION DOTS ── -->

    <!-- AP, India (HQ) -->
    <circle cx="${hqX.toFixed(2)}" cy="${hqY.toFixed(2)}" r="10" fill="#39B54A" opacity="0.25"/>
    <circle class="dot-green-ring" cx="${hqX.toFixed(2)}" cy="${hqY.toFixed(2)}" r="6" fill="#39B54A" opacity="0.9"/>
    <circle cx="${hqX.toFixed(2)}" cy="${hqY.toFixed(2)}" r="3" fill="#ffffff"/>

    <!-- Nepal -->
    <circle cx="${neX.toFixed(2)}" cy="${neY.toFixed(2)}" r="8" fill="#FDB813" opacity="0.25"/>
    <circle class="dot-yellow-ring" cx="${neX.toFixed(2)}" cy="${neY.toFixed(2)}" r="4.5" fill="#FDB813" opacity="0.9"/>
    <circle cx="${neX.toFixed(2)}" cy="${neY.toFixed(2)}" r="2" fill="#ffffff"/>

    <!-- Sri Lanka -->
    <circle cx="${slX.toFixed(2)}" cy="${slY.toFixed(2)}" r="8" fill="#FDB813" opacity="0.25"/>
    <circle class="dot-yellow-ring" cx="${slX.toFixed(2)}" cy="${slY.toFixed(2)}" r="4.5" fill="#FDB813" opacity="0.9"/>
    <circle cx="${slX.toFixed(2)}" cy="${slY.toFixed(2)}" r="2" fill="#ffffff"/>

    <!-- Philippines -->
    <circle cx="${phX.toFixed(2)}" cy="${phY.toFixed(2)}" r="8" fill="#FDB813" opacity="0.25"/>
    <circle class="dot-yellow-ring" cx="${phX.toFixed(2)}" cy="${phY.toFixed(2)}" r="4.5" fill="#FDB813" opacity="0.9"/>
    <circle cx="${phX.toFixed(2)}" cy="${phY.toFixed(2)}" r="2" fill="#ffffff"/>
